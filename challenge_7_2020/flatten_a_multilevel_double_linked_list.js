 ...
 */
const flatten = head => {
  let list = mergeList(head);
  return list[0];
};


const mergeList = node => {
  let firstNode = node;
  let lastNode = null;
  ...
  while (node) {
    lastNode = node;
    if (node.child) {
      let nodePair = mergeList(node.child)
      let oldNext = node.next;
      node.next = nodePair[0];
      nodePair[0].prev = node;
      if (oldNext) {
        nodePair[1].next = oldNext;
        oldNext.prev = nodePair[1];
      }
      node.child = null;
      node = oldNext;
    } else {

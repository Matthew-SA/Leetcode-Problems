...
const leastInterval = (tasks, n) => {
  ...
  tasks.forEach(task => taskCounts[task] ? taskCounts[task] += 1 : taskCounts[task] = 1);

  let numOfTasks = tasks.length;
  let queue = (Object.values(taskCounts)).sort((a, b) => b - a);
  let cooldownQueue = new Array(n).fill(null);
  let cycleCount = 0;
  ...
  while (numOfTasks) {
    ...
    if (currentTask) {
      cooldownQueue.push(currentTask - 1)
      numOfTasks--;
    } else {
      cooldownQueue.push(null);
    }

    let cooldownTask = cooldownQueue.shift();
    if (cooldownTask) queue.unshift(cooldownTask);
    if (queue[0] < queue[1]) queue.sort((a, b) => b - a);
